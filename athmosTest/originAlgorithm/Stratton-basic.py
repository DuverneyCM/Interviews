...
class OrdersManager:
    ...
    def __fake_save_on_db(self, order):
        id, number = order
        self.__log(message=f"Order [{id}] {number} was successfully prosecuted.")
    ...
    def process_orders(self):
        for order in self.__orders:
            ...
            if datetime.now() > self.__last_printed_log:
                ...
                self.__log(message=f"Total orders executed: {self.__orders_processed}/{len(self.__orders)}")
...
print(f"{datetime.now()} > Tiempo de ejecución: {delay} segundos...")